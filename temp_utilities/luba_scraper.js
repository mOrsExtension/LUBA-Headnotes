...
class LUBAScraper {
  ...
  async getAvailableYears(url) {
    ...
    try {
      ...
      console.log(
        `  Found ${yearLinks.length} years: ${yearLinks
          .map((y) => y.year)
          .slice(0, 10)
          .join(", ")}${yearLinks.length > 10 ? "..." : ""}`,
      );
      return yearLinks;
    } catch (error) {
  ...
  async scrapeYear(yearData, sourceType = "opinions") {
    ...
    try {
      ...
      const opinions = await this.page.evaluate(
        (year, sourceType) => {
          ...
          lines.forEach((line) => {
            ...
            if (monthHeaders.includes(line)) {
              currentMonth = String(monthHeaders.indexOf(line) + 1).padStart(
                2,
                "0",
              );
              return;
            ...
            const casePattern =
              /\[?(\d{2,4}-\d\S+)\]?\s*(.*?v\.?.*?)(?:,\s*(\d+\s+Or\s+LUBA\s+\d+)\s*\(\d{4}\)|\s*\(Unpublished\)|$)/;
            ...
            if (match && currentMonth) {
              ...
              for (const link of links) {
                if (
                  link.href &&
                  (link.href.includes(lubaNoClean) ||
                    link.textContent.includes(lubaNo))
                ) {
                  url = link.href;
                  break;
                }
      ...
      const domOpinions = await this.page.evaluate(
        (year, sourceType) => {
          const results = [];
          const monthMap = {
            January: "01",
          ...
          const links = Array.from(
            document.querySelectorAll('a[href*="/luba/Docs/"]'),
          );
          ...
          links.forEach((link) => {
            ...
            if (lubaIndex !== -1) {
              contextText = contextText
                .substring(lubaIndex + lubaNo.length)
                .trim();
            }
            ...
            if (contextText.includes("(Unpublished)")) {
              ...
              const reporterMatch = contextText.match(
                /^(.*?),\s*(\d+\s+Or\s+LUBA\s+\d+)\s*\(\d{4}\)/i,
              );
              if (reporterMatch) {
      ...
      const yearOpinions =
        domOpinions.length >= opinions.length ? domOpinions : opinions;

      console.log(
        `  Found ${yearOpinions.length} ${sourceType} for ${yearData.year}`,
      );
      ...
      console.error(
        `Error scraping year ${yearData.year} for ${sourceType}:`,
        error.message,
      );
      return [];
    }
  ...
  async scrapeAllYears(startYear = null, endYear = null) {
    ...
    for (const url of this.pageUrls) {
      ...
      try {
        ...
        console.log(
          `Will scrape ${yearsToScrape.length} years for ${sourceType}`,
        );
...
async function main() {
  ...
  try {
    ...
    if (yearFirst && yearLast && yearFirst < yearLast) {
      await scraper.scrapeAllYears(yearFirst, yearLast); //
    } else {
