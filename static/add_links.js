document.addEventListener("DOMContentLoaded", async () => {
  ...
  headNumLinks.forEach((headNumLink) => {
    ...
    headNumLink.setAttribute(
      "title",
      `Search for headnote ${headNumLink.textContent}>`,
    );
  });
  ...
  headnoteEntries.forEach((headnoteEntry) => {
    ...
    if (headnoteHeader) {
      ...
      if (newHeadnoteNum.length > 0) {
        headnoteHeader.innerHTML = headerText.replace(
          headnoteNum,
          newHeadnoteNum,
        );
      }
    ...
    metadataLines.forEach((line) => {
      ...
      if (lineText.includes("LUBA No.:")) {
        const lubaNumMatch = lineText.match(
          /(\d{4})-(\d+)(\/\d+)?(\/\d+)?(\/\d+)?(\/\d+)*/,
        );
        if (lubaNumMatch) {
          ...
          lubaNumMatch.forEach((aMatch, index) => {
            if (index > 1 && aMatch) {
              ...
              replaceString = replaceString.replace(
                caseNumRE,
                `<a href="${link}" class="luba_num_link">${caseNum}</a>`,
              );
            }
          });
